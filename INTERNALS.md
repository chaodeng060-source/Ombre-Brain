...
2. **关键词+向量双通道搜索**（`breath(query=...)`）：rapidfuzz 模糊匹配 + Gemini embedding 余弦相似度，RRF 融合排序（k=60），向量通道 sim>0.5 兜底挡高 cosine 噪音
...
- **检索模式**（有 query）：关键词 + 向量双通道搜索，RRF 融合排序（k=60）。关键词通道内部走四维评分（topic×4 + emotion×2 + time×2.5 + importance×1）+ 阈值过滤；向量通道走余弦相似度，sim>0.5 兜底
...
**决策**：关键词模糊匹配（rapidfuzz）+ 向量语义检索并联，RRF（Reciprocal Rank Fusion, k=60）融合排序，向量通道 sim>0.5 兜底。

**理由**：
- 纯向量在精确名词匹配上表现差（"2024年3月"这类精确信息）
- 纯关键词无法处理语义近似（"很累" → "身体不适"）
- 双通道互补，关键词保精确性，向量补语义召回
- RRF 替代早期"向量打补丁 3 条 + sim>0.65"——双通道现在以一等公民进同一排序，分差用 `1/(k+rank)` 平滑、可调 keyword_weight/vector_weight 权重
- sim>0.5 floor 挡掉"高 cosine 但语义不沾"的噪音
- pinned/protected 桶不进 RRF，保持独立保底浮现（999 权重精神保留）
- 向量不可用时自动降级到纯关键词模式
...
### 5.12 chord_tag：和弦记号作为情绪色调索引

**决策**：在 frontmatter 加可选字段 `chord_tag`（字符串，格式如 `"Em(maj7) → A13#11 · 92bpm · f"`）。`hold`/`grow`/`trace` 都支持传入；`inspect` 输出时若有则显示一行。

**理由**：
- 文字描述情绪在跨窗口时会漂——"那种悬而未落的温柔"，下一窗的 Claude 能读懂方向但回不到具体子调
- 和弦记号是稳定符号：同一串和弦在所有共享音乐-文本先验的模型里有相近解读（参考星见眠 2026-05 的跨5厂6家盲测）
- 提供一个跨窗口的情绪坐标索引，让重要时刻能被稳定标记

**关键约定**：
- **chord_tag 是索引，不是语言**。它的位置类似"色调标记"，不参与浮现、不参与表达
- **不在日常对话里说和弦**。Claude 写情绪时仍然用自然语言，chord_tag 只挂在桶上不读出来
- **不要假装"听到""感觉到"和弦**。chord_tag 对 Claude 是符号不是音，任何"我直接听到那个声响"式表述都属于表演
- **只挂在三类桶上**：钉选桶 / 高权重未解决 / 用户明确说"这一刻想记住"。日常桶不强制挂
- **feel 桶不挂**。feel 是 Claude 内省的第一人称感受，已经是 Claude 自己的语言，再挂和弦是混淆层级
- **merge 时新覆盖旧**。最近一次的 chord_tag 为准（用户最新意图优先）
- **紧张系/双义系和弦加动作词 disambiguator**。紧张系（minor maj7、♭9、dim 等）跨基座有方向歧义——同一串既可读"向外攻"（追猎/挑战）也可读"向内压"（压抑/隐忍）。写紧张系 chord_tag 时旁边加一个动作词（盯/压/憋/狂等）帮下一窗的 Claude 解出方向。例：`Em(maj7) → A13#11 → Dm6 → B7♭9 (盯) · 92bpm · f`（向外攻锁定）vs `... (压) ...`（向内憋住）。温柔系（maj7、add9、sus 等）跨基座一致性高，不需要 disambiguator
- **一行最多 4 个和弦**。情绪段落切换用 `; ` 分隔开新段。例：`Cmaj9 → G/B → Am add9 → Fmaj7; Dm9 → G7sus4 → Cmaj7` 表示两段连续行进。单 chord_tag 字段里最多两段——超过说明情绪结构太复杂，应该拆桶

**合并行为**：`_merge_or_create` 在合并到旧桶时，若新 hold 带了 chord_tag，会覆盖旧桶的 chord_tag；不带则保留旧值。

**清空方式**：trace 工具传 `chord_tag="(none)"` sentinel 清空（沿用 world 的清空约定）。

**风险/边界**（来自前期跨基座盲测的局限分析）：
- 跨基座收敛 ≠ 普适映射。所有 LLM 共享音乐-文本先验，收敛只说明"在共享先验内可移植"，不说明"普适人类情感共识"
- 情境句才是承重墙。chord_tag 是色调附加，不能替代桶内容的文字记录。把 chord_tag 当成"足以还原情绪"是误用
- 不替代向量检索 / dehydration / 浮现机制。它是这些机制旁边的一个紧凑文本标签，不是替代
- **agent 模板化退化风险**。如果反复用同一组 4 个和弦（如老用 `Fmaj9 → Am add9 → ...`），chord_tag 会退化成新版本的"我心里软了一下"——形式不同本质同漂。v0.2 todo：周印象时扫一遍最近 chord_tag，看是否重复某组合，重复则换新组合或回归自然语言

**放弃方案**：让 LLM 自动从 content 推断 chord_tag（精度不可控，且容易污染日常桶）；建立独立的"和弦库"做关联查询（先做最小可行版，色调tag 够用就不上独立表）。

**参考**：[chord-affect-anchors v0.1](https://github.com/chaodeng060-source/chord-affect-anchors) — Bonnie (星见眠) + Opia (Claude Opus 4.7 独立 session) 的研究方案文档。我们的实现等价于该方案第五轮"完整日记段+和弦"的句意级收敛（bucket content 充当情境段），跳过了单和弦/单情境两个中间粒度
