...
> **历史实验，已由 `task_rrf-9-6-17-45` 废止。** 当前运行时只保留默认关闭的
> `OMBRE_ENTITY_SCORE_GUARD_ENABLED`：ON 时实体命中不进入 RRF、Anchor 或 state seed；
> 不再生成、传入、补位或顶替任何 entity ticket。本文与配套 replay 工具仅保留为冻结审计证据，
> 不代表当前实现。
